...
plot4 <- joined_bed_data %>%
  select(location_name, percentage_occupancy, season, quarter, quarter_year) %>% 
  filter(location_name == "Perth Royal Informary" & quarter_year == "2018") %>% 
  #You can add a filter column her by location name linked to ui, allow multiple selections.
  group_by(quarter) %>% 
  summarise(quarter = quarter, 
            average_bed_occupancy = round(mean(percentage_occupancy, na.rm =TRUE)))%>%  
