...
ggplotly(plot3)
```
plot 4 - Average Occupancy - All Hospital by Quarter

```{r}

plot4 <- joined_bed_data %>%
  mutate(ts_quarter = yearquarter(quarter)) %>% # turn into regular date
  filter(!is.na(ts_quarter)) %>% 
  ...
  group_by(ts_quarter, location_name) %>% 
  summarise(location_total = n(), 
            average_bed_occupancy = round(sum(percentage_occupancy, na.rm =TRUE)/ 
                                        location_total)) %>% 
  ggplot()+
  aes(x = ts_quarter, y = average_bed_occupancy, group = location_name, colour = location_name)+
  geom_line()+
  geom_point()+
